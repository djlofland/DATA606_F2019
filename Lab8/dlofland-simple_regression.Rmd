...
Source files: [https://github.com/djlofland/DATA606_F2019/tree/master/Lab8](https://github.com/djlofland/DATA606_F2019/tree/master/Lab8)
...
> Scatter plot, visually appears like a linear relationship, yes, but there is quite a bit of variability.
...
> There is a general positive relationship, as at_bats increases, runs increases.  The strength is moderate
...
>  Smallest: 144,959.9, most sums were ~200,000 this was slightly smaller
...
> ~725 runs, overestimate ... the closest datapoint is at 5579 with a value of 713.  We are overestimating by 12 (residual = 725-713 = 12)
...
> there does NOT appear to be any clear pvisual atterns to the residuals.  There are more datapoints with lower at_bats values, so we probably have more confidence with lower values.  The couple points to the right might be outliers, but the residuls still fall within reasonable distance from 0.
...
> Based on the qqplot, there are two extreme points (one high, one low) that might be treated as outliers giving a closer fit to normal with the bulk of data.  That said, this is a reasonable approximation of norrmal.
...
> I chose `hits`, based on the scatter, there appear to be a positive linear relationship
...
> Using `hits` we get an Adjusted R$^2$ of 0.6292 vs 0.3505 for `at_bats`.  `hits` better predicts `runs` and explains more of the variability.
...
```{r on_own3}
#m4 <- lm(runs ~ at_bats + hits + homeruns + bat_avg + strikeouts + stolen_bases + wins, data=mlb11)
#summary(m4)
...
> The new variables are far better predictors of `runs` with Adjusted R$^2$ of `new_obs` of  0.93, `new_slug` of 0.8932 and `new_onbase` of 0.8437.  `new_obs` has the best predicted value of all 10 variables.  `new_obs` is a combination of both slugging and on-base which both had reall good R$^2$.  The combination explains even more of the variability and this makes sense.
...
summary(m6)
plot(m6$residuals ~ mlb11$new_obs)
abline(h = 0, lty = 3)  # adds a horizontal dashed line at y = 0
hist(m6$residuals)
qqnorm(m6$residuals)
...
> residuals are normal, no trends and have variable spread
