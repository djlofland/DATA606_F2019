...
Source files: [https://github.com/djlofland/DATA606_F2019/tree/master/Lab2]
...
```{r}
library(tidyverse)
library(formattable) # percent() method to render percentages
library(aplpack)     # bagplot() method to render bagplots
```
...
> There are **`r x[1]`** observations and **`r x[2]`** variables in the cdc dataset.  

Variable   |  Datatype
---------- | ------------
genhlth    | Factor (Categorial, ordinal)
exerany    | Numeric (Float, logical)
hlthplan   | Numeric (Float, logical)
smoke100   | Numeric (Float, logical)
height     | Numeric (Float, Discrete)
weight     | Integer (Discrete)
wtdesire   | Integer (Discrete)
age        | Integer (Discrete)
gender     | Factor (Categorial, nominal)
...
```{r}
# Basic info on height
summary(cdc$height)

# Basic info on age
summary(cdc$age)

# Get the interquartile range for height
IQR(cdc$height)

# Get the interquartile range for age
IQR(cdc$age)

# Gender .. gender frequency distribution
table(cdc$gender)

# Exercise (Any) ... exerany frequency distribution
table(cdc$exerany)

# General Health Proportions 
table(cdc$genhlth) / dim(cdc)[1]
genhealthProp <- prop.table(table(cdc$genhlth))
excellentHealth <- formattable::percent(genhealthProp[1], 2)
```

> There are **`r table(cdc$gender)[1]`** males in the sample and **`r excellentHealth`** report to be in excellent health.
...
> A higher portions of males report having smoked compared with females.
...
> The boxplot shows a trend that as reported health quality decreases, we see an increase in the BMI (median and IQR range) along with generally higher outliers.  Note that there are a few outliers with high BMI that report good or very good health.  It might be interesting to explore whether other factors might help explain perceptions fo health versus the basics stats of health.
...
> I looked at age, smoking and exercise.  I would hypothesize that highe BMI might lead to general stress and higher smoking.  With exercise, I would hypothesize that people reporting exercise might have lower BMI and finally, with age, my guess is that BMI would increase during early life (18-30) at which point people get concened with health (30+) and maybe start making life changes.  Higher BMI is likely associated with morbidity, so as the population ages, I would expect the BMI to generally decrease as our sample has fewer high BMI individuals still around.

> Smoking: Intersting, the BMI(median and IQR) are very similar between smokersna dn non-smoker, but we see fewer high outliers with smokers and more outliers in the low BMI range.  This might suggest smoking does have a positive association with BMI for some individual.  Since smoking100 only indicates yes/no to 100 cigarettes, we don't know how how much these people smoked - possibly the amount of smoking realtes to BMI - that would be a good followup question to t explore.

> Exercise: There is a slight decrease in BMI (median, IQR and high outliers) with the group that reports exercise.  This fits with my expectations.  Agin, we don't know how much poeple exercised so I would want to further explore that ans segment activity levels and look for correlations with BMI.

> Age: BMI (median, IQR and high outliers) generally increase over age until ~60's when we see it decreasing.  High outliers start decreasing ~ age 50. We would want to further explore the number of people at those ages to see if we are selectively loosing high BMI thus decreasing the stats or whether people are making life changes later in life that decrease these stats.
...
```{r}
# Plot weight vs desired weight
scatter.smooth(cdc$weight, cdc$wtdesire)

# plot without outliers
remove_outliers <- subset(cdc, cdc$wtdesire < 400)
scatter.smooth(remove_outliers$weight, remove_outliers$wtdesire)

# calculated the difference between weight and desired weight per person
weight_delta <- remove_outliers$weight - remove_outliers$wtdesire
hist(weight_delta, breaks = 100)
scatter.smooth(remove_outliers$weight, weight_delta, xlab='Current Weight', ylab='Weight Loss desired')
boxplot(weight_delta ~ remove_outliers$weight, main="Desired Weigh Loss by weight", xlab='Current Weight', ylab='Weight Loss desired')
aplpack::bagplot(remove_outliers$weight, weight_delta, main="Bagplot of weight vs desired weight loss", xlab='Current Weight', ylab='Weight Loss desired')
```

> There were a few strange outliers where people reported wanting an extremely high weight.  I'm removing them to focus on the broader sample.  Genreally, as weight increases, so does the desired weight loss (current weight - desired weight).  For weights < 200, there are more people reporting a desired weight at or higher than their current weight (ie they are happy or want to gain a little). As weight increases above 200 lbs, people report wanting to loose more weight.
...
> I didn't read this question before answering the one above.  Oops. Please see ***weight_delta** as it's equivalent to **wdiff*.

```{r}
wdiff <- cdc$weight - cdc$wtdesire
wdiff_no_outliers <- remove_outliers$weight - remove_outliers$wtdesire 
```
...
> **wdiff** is an integer representing the difference between a persons current weight and desired weight (ie how much weight do they want to loose or gain).  If *wdiff* is **positive** they want to **loose** that weight.  When *wdiff* is **negative**, they want to **gain** weight. When *wdiff* is **0**, they are **happy** with their current weight.
...
> The median desired weight loss is 10 lbs with a mean at 14.59 lbs.  This suggests a skewed distribution (long tail) where there are people reporting higher desired weight loss. Also see the box and bagplots above with my commentary - *wdiff* increase with actual weight, esp as current weight increases over 200 lbs.

-   Using numerical summaries and a side-by-side box plot, determine if men tend
    to view their weight differently than women.

```{r}
boxplot(wdiff_no_outliers ~ remove_outliers$gender)
```

> Men report wanting to loose less weight than women and we see a number of outliers suggesting men wanting to gain weight.  This might suggest men wanting to gain muscle mass vs just weight in general (a followup area to explore).  Women report wanting to loose more weight (media, IQR and upper outlier range).
...
cdc$zscore <- round((cdc$weight - weight_mean) / weight_sd, 1)
#table(cdc$zscore)
z_prop <- prop.table(table(cdc$zscore))

cols_within_1z <- as.character(as.numeric(names(z_prop))[abs(as.numeric(names(z_prop))) <= 1])
within_1_zscore <- formattable::percent(sum(z_prop[cols_within_1z]), 2)

cols_less_1z <- as.character(as.numeric(names(z_prop))[as.numeric(names(z_prop)) < -1])
less_1_zscore <- formattable::percent(sum(z_prop[cols_less_1z]), 2)

cols_greater_1z <- as.character(as.numeric(names(z_prop))[as.numeric(names(z_prop)) > 1])
greater_1_zscore <- formattable::percent(sum(z_prop[cols_greater_1z]), 2)

hist(cdc$zscore)
```

> **`r within_1_zscore`** of people are within $\pm1$ SD of mean. **`r less_1_zscore`** of people are less than -1 SD and **`r greater_1_zscore`** are greater than 1 SD.  Since a z_scores distribution is by definition normal, I expected the portion over and under to be equal.  I calcualted them just in case as a validation check to make sure I properly summed the within $\pm1$ range.