...
Source files: [https://github.com/djlofland/DATA606_F2019/tree/master/Lab3](https://github.com/djlofland/DATA606_F2019/tree/master/Lab3)
...
```{r first9, eval=TRUE}
kobe$basket[1:9]
kobe <- kobe %>% group_by(game, quarter) %>% mutate(time_lead = lead(time, n=1, default=NA)) 

kobe$time <- ms(kobe$time)
kobe$time_lead <- ms(kobe$time_lead)
kobe$delta <- kobe$time - kobe$time_lead
kobe$delta <- int_length(seconds(kobe$delta))
kobe
```
...
> It looks like a "streak" is defined as attempted **shots made within ~1 minute of each other**, whether the shot missed or hit.  I added a new column *kobe$delta* which contain the time difference between the current shot and the next shot attempt.  
...
distribution.
```{r}
barplot(table(kobe$delta[kobe$delta<=60]))
hist(table(kobe$delta[kobe$delta<=60]), breaks=5)
```

> **NOTE: My plots don't match the calc_streak() output exactly, so I'm guessing there are other rules for what a "streak" is that I don't fully understand.**
...
> This looks like a typical **exponential or logrithmic dropoff/decay** curve.  His longest streak was **4 shots**.
...
> I ran it a number of times and saw: **13, 20,1 8, 22, 26**
...
> sim_streak shows a similar exponential dropoff with the majority of streak values of 0, 1 and 2 and a long narrow tail.  The longest streaks were 5 long, but with very few occurances.
...
> I would expect repeated runs to be a similar shape but with some random variation.  Since the tail is quite narrow, I'm guessing we would see more opvious differences in the max streaks.  Since the simulation was randomized, it's possbile to see edge cases.  If we were to run the 133 shot simulation a number of times and average the results, we would have more confidence in the general shape for comparison.  That said, maybe its ok for simulations to off days just like real basketball players ;)
...
> sim_streak shows a similar exponential dropoff, but has far more 0's than Kobe and narrower right tail compared with Kobe.  Kobe's curve shows a higher portion of 1, 2 & 3 streaks compared with the simulation.  It's possible this was an anomoly with this particular run of the simulation, but comparing this simulation to Kobe, there are noticable differences.  I would lean towards there being some effect leading to more 1, 2 and 3 streaks with Kobe vs simulation.  Note that it could be an effect happening to the opposing team or even Kobe's team.  Maybe getting a basket has an effect on the opponent lowering their performance and giving Kobe a greater probability of getting a streak?  Thus far, I cannot say specifically that Kobe is the cause for the change in streak, but there does appear to be a "hot-hands" effect going on as seen in this dataset.