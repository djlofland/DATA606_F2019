...
> There is a genenal positive trend upwards in the number of girls baptized each year with a notable drop between 1640~1665.  It would appear something occurred during these years that either affected record keeping and/or the number of girls baptized.  Overall the slope increases from ~4500 girls in 1630 to ~7500 girls in 1710.
...
> There appears to be a slight downward trend in the portion of boys being born/baptized over time.  This could suggest there are fewer boys, more girls or some other factor affecting data gathering.
...
```{r present-question-1, eval=TRUE}
present
dim(present)
names(present)
```

> This dataset includes **1940-2002**, the dimesions are: **63 rows and 3 columns ** and the variables are: **year, boys, girls** 

-   How do these counts compare to Arbuthnot's? Are they on a similar scale?

```{r present-question-2, eval=TRUE}
mean(present$boys + present$girls) / mean(arbuthnot$boys + arbuthnot$girls)
median(present$boys + present$girls) / median(arbuthnot$boys + arbuthnot$girls)
```

> The number of children per year is about **~320x larger** than during Arbuthnot's time. Also note that Arbuthnot tracked **82 years** while the present dataset include **63 years**. 
...
```{r present-question-3, eval=TRUE}
plot(present$year, (present$boys/(present$boys + present$girls)))
```
    
> Both in Arbuthnot's time and present, the portion of boys is greater than girls for every year centering just above ~51%.  I do notice greater variability with Arbuthnot's data ... the percentage of boys per year ranges between 51% and 53%.  With the current data, the spread is narrowed between 51.15% and 51.35%, centering ~51.2%
...
```{r present-question-4, eval=TRUE}
max_births <- max(present$boys + present$girls)
max_year <- present$year[which.max(present$boys + present$girls)]
```
> The year with the most births was **`r max_year`**, with **`r format(max_births, big.mark=",", scientific=FALSE)`** births. 
